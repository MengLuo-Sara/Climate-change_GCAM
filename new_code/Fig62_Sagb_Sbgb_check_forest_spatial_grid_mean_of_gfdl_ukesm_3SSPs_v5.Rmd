...
## gfdl ####################################
agb_file_SSP126_gfdl <- read.csv(paste0(main_Sagb_path, "classic_gfdl-esm4_w5e5_ssp126_2015soc-from-histsoc_default_grid_scale_forest.csv"))%>%
  dplyr::select(center_lon,center_lat,npp)
agb_file_SSP370_gfdl <- read.csv(paste0(main_Sagb_path, "classic_gfdl-esm4_w5e5_ssp370_2015soc-from-histsoc_default_grid_scale_forest.csv"))%>%
  dplyr::select(center_lon,center_lat,npp)
agb_file_SSP585_gfdl <- read.csv(paste0(main_Sagb_path, "classic_gfdl-esm4_w5e5_ssp585_2015soc-from-histsoc_default_grid_scale_forest.csv"))%>%
  dplyr::select(center_lon,center_lat,npp)


bgb_file_SSP126_gfdl <- read.csv(paste0(main_Sbgb_path, "classic_gfdl-esm4_w5e5_ssp126_2015soc-from-histsoc_default_grid_scale_forest.csv"))%>%
  dplyr::select(center_lon,center_lat,rh_npp)
bgb_file_SSP370_gfdl <- read.csv(paste0(main_Sbgb_path, "classic_gfdl-esm4_w5e5_ssp370_2015soc-from-histsoc_default_grid_scale_forest.csv"))%>%
  dplyr::select(center_lon,center_lat,rh_npp)
bgb_file_SSP585_gfdl <- read.csv(paste0(main_Sbgb_path, "classic_gfdl-esm4_w5e5_ssp585_2015soc-from-histsoc_default_grid_scale_forest.csv"))%>%
  dplyr::select(center_lon,center_lat,rh_npp)
...
# forcing_name2<-"ukesm_"
  
agb_file_SSP126_ukesm <- read.csv(paste0(main_Sagb_path, "classic_ukesm1-0-ll_w5e5_ssp126_2015soc-from-histsoc_default_grid_scale_forest.csv"))%>%
  dplyr::select(center_lon,center_lat,npp)%>%
   distinct()
agb_file_SSP370_ukesm <- read.csv(paste0(main_Sagb_path, "classic_ukesm1-0-ll_w5e5_ssp370_2015soc-from-histsoc_default_grid_scale_forest.csv"))%>%
  dplyr::select(center_lon,center_lat,npp)%>%
   distinct()
agb_file_SSP585_ukesm <- read.csv(paste0(main_Sagb_path, "classic_ukesm1-0-ll_w5e5_ssp585_2015soc-from-histsoc_default_grid_scale_forest.csv"))%>%
  dplyr::select(center_lon,center_lat,npp)%>%
   distinct()


bgb_file_SSP126_ukesm <- read.csv(paste0(main_Sbgb_path, "classic_ukesm1-0-ll_w5e5_ssp126_2015soc-from-histsoc_default_grid_scale_forest.csv"))%>%
  dplyr::select(center_lon,center_lat,rh_npp)%>%
   distinct()
bgb_file_SSP370_ukesm <- read.csv(paste0(main_Sbgb_path, "classic_ukesm1-0-ll_w5e5_ssp370_2015soc-from-histsoc_default_grid_scale_forest.csv"))%>%
  dplyr::select(center_lon,center_lat,rh_npp)%>%
   distinct()
bgb_file_SSP585_ukesm <- read.csv(paste0(main_Sbgb_path, "classic_ukesm1-0-ll_w5e5_ssp585_2015soc-from-histsoc_default_grid_scale_forest.csv"))%>%
  dplyr::select(center_lon,center_lat,rh_npp)%>%
   distinct()
...
value_min <- min(
  agb_file_SSP126_gfdl$npp,
  agb_file_SSP370_gfdl$npp,
  agb_file_SSP585_gfdl$npp,
  bgb_file_SSP126_gfdl$rh_npp,
  bgb_file_SSP370_gfdl$rh_npp,
  bgb_file_SSP585_gfdl$rh_npp,
  agb_file_SSP126_ukesm$npp,
  agb_file_SSP370_ukesm$npp,
  agb_file_SSP585_ukesm$npp,
  bgb_file_SSP126_ukesm$rh_npp,
  bgb_file_SSP370_ukesm$rh_npp,
  bgb_file_SSP585_ukesm$rh_npp,
  na.rm = TRUE
)
value_max <- max(agb_file_SSP126_gfdl$npp,
  agb_file_SSP370_gfdl$npp,
  agb_file_SSP585_gfdl$npp,
  bgb_file_SSP126_gfdl$rh_npp,
  bgb_file_SSP370_gfdl$rh_npp,
...
# Extract the range for x (longitude) and y (latitude)
x_length <- bbox["xmax"] - bbox["xmin"]  # Length in the x direction (longitude)
y_length <- bbox["ymax"] - bbox["ymin"]  # Length in the y direction (latitude)

# Print the results
cat("X direction (longitude) length:", x_length, "\n")
cat("Y direction (latitude) length:", y_length, "\n")
...
agb_file_SSP126_mean<-agb_file_SSP126_gfdl%>%
  rename(gfdl=npp)%>%
  left_join(agb_file_SSP126_ukesm%>%rename(ukesm=npp))%>%
  mutate(mean=(ukesm+gfdl)/2)%>%
   distinct()


not_NA_col_num <- which(is.na(agb_file_SSP126_mean$mean))

write.csv(not_NA_col_num,"not_NA_col_num.csv")

write_rds(agb_file_SSP126_mean,"agb_file_SSP126_mean.RDS")

agb_file_SSP370_mean<-agb_file_SSP370_gfdl%>%
  rename(gfdl=npp)%>%
  left_join(agb_file_SSP370_ukesm%>%rename(ukesm=npp))%>%
  mutate(mean=(ukesm+gfdl)/2)%>%
   mutate(mean=ifelse(is.na(mean),1,mean))%>%
   distinct()%>%
  mutate(mean = ifelse(row_number() %in% not_NA_col_num, NA, mean))

agb_file_SSP585_mean<-agb_file_SSP585_gfdl%>%
  rename(gfdl=npp)%>%
  left_join(agb_file_SSP585_ukesm%>%rename(ukesm=npp))%>%
  mutate(mean=(ukesm+gfdl)/2)%>%
   mutate(mean=ifelse(is.na(mean),1,mean))%>%
   distinct()%>%
  mutate(mean = ifelse(row_number() %in% not_NA_col_num, NA, mean))
...
bgb_file_SSP126_mean<-bgb_file_SSP126_gfdl%>%
  rename(gfdl=rh_npp)%>%
  left_join(bgb_file_SSP126_ukesm%>%rename(ukesm=rh_npp))%>%
  mutate(mean=(ukesm+gfdl)/2)%>%
   mutate(mean=ifelse(is.na(mean),1,mean))%>%
   distinct()%>%
  mutate(mean = ifelse(row_number() %in% not_NA_col_num, NA, mean))

bgb_file_SSP370_mean<-bgb_file_SSP370_gfdl%>%
  rename(gfdl=rh_npp)%>%
  left_join(bgb_file_SSP370_ukesm%>%rename(ukesm=rh_npp))%>%
  mutate(mean=(ukesm+gfdl)/2)%>%
   mutate(mean=ifelse(is.na(mean),1,mean))%>%
   distinct()%>%
  mutate(mean = ifelse(row_number() %in% not_NA_col_num, NA, mean))

bgb_file_SSP585_mean<-bgb_file_SSP585_gfdl%>%
  rename(gfdl=rh_npp)%>%
  left_join(bgb_file_SSP585_ukesm%>%rename(ukesm=rh_npp))%>%
  mutate(mean=(ukesm+gfdl)/2)%>%
   mutate(mean=ifelse(is.na(mean),1,mean))%>%
   distinct()%>%
  mutate(mean = ifelse(row_number() %in% not_NA_col_num, NA, mean))
###############################
  ...
  # border <- ggplot(agb_file_SSP126_mean, aes(x = center_lon, y = center_lat, fill = mean)) +
  #     scale_fill_gradientn(
  #     # colors = c("#FF6503", "#ffffb2", "#2BA428"),
  #     colors = c("#D33300", "#ffffb2", "#169B13"),
  #     values = rescale(c(value_min, 1, value_max)), # Normalize the values to include min, 0, and max
...
plot_agb_SSP126_mean <- plot_spatial_map(agb_file_SSP126_mean, value_min, value_max,if_ytitle=TRUE,ifxlabel=FALSE)

plot_agb_SSP370_mean <- plot_spatial_map(agb_file_SSP370_mean, value_min, value_max,if_ytitle=TRUE,ifxlabel=FALSE)

plot_agb_SSP585_mean <- plot_spatial_map(agb_file_SSP585_mean, value_min, value_max,if_ytitle=TRUE,ifxlabel=TRUE)



plot_bgb_SSP126_mean <- plot_spatial_map(bgb_file_SSP126_mean, value_min, value_max,if_ytitle=FALSE,ifxlabel=FALSE)

plot_bgb_SSP370_mean <- plot_spatial_map(bgb_file_SSP370_mean, value_min, value_max,if_ytitle=FALSE,ifxlabel=FALSE)

plot_bgb_SSP585_mean <- plot_spatial_map(bgb_file_SSP585_mean, value_min, value_max,if_ytitle=FALSE,ifxlabel=TRUE)





plot_legend<-plot_spatial_map(agb_file_SSP585_mean, value_min, value_max, TRUE)
...
combined_agb_plots <- wrap_plots(plot_agb_SSP126_mean,plot_agb_SSP370_mean,plot_agb_SSP585_mean, nrow = 3)
combined_bgb_plots <- wrap_plots(plot_bgb_SSP126_mean,plot_bgb_SSP370_mean,plot_bgb_SSP585_mean, nrow = 3)
...
ggsave(paste0(fig_save_path, "/Fig62_3SSP_combined_Sagb_bgb_spatial_2forcing_mean_v",version_num,".pdf"), plot = combined_global_bgb_plot, width = 15*0.8, height = 10*0.8, units = "in")


# ggsave(paste0(fig_save_path, "/Fig62_3SSP_combined_Sagb_bgb_spatial_2forcing_mean_bounder_v",version_num,".pdf"), plot = border, width = 8*0.8, height = 4*0.8, units = "in")
# 
# ggsave(paste0(fig_save_path, "/Fig62_3SSP_combined_Sagb_bgb_spatial_2forcing_mean_bounderx_v",version_num,".pdf"), plot = borderx, width = 8*0.8, height = 4*0.8, units = "in")
# 
# ggsave(paste0(fig_save_path, "/Fig62_3SSP_combined_Sagb_bgb_spatial_2forcing_mean_boundery_v",version_num,".pdf"), plot = bordery, width = 8*0.8, height = 4*0.8, units = "in")


#  ggsave(paste0(fig_save_path, "/Fig62_3SSP_combined_Sagb_bgb_spatial_2forcing_mean_v",version_num,".svg"), plot = combined_global_bgb_plot , width = 15*0.8, height = 10*0.8, units = "in")
# ggsave(paste0(fig_save_path, "/Fig62_3SSP_combined_Sagb_bgb_spatial_2forcing_mean_v",version_num,".png"), plot = combined_global_bgb_plot, width = 15*0.8, height = 10*0.8, units = "in", dpi = 300)
# ggsave(filename = paste0(fig_save_path,"/Fig62_3SSP_combined_Sagb_bgb_spatial_2forcing_mean_v", version_num, ".tif"),
