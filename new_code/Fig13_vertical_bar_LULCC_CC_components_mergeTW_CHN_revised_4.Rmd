...
land.alloc.data.table_ssp_default_updateall <- read.csv(paste0(input_dir,"aggregated.land.allocation.",scenario,"_v3_classic_gfdl_default_updateall_fixed.csv"))%>%
  mutate(scenario="default_updateall")

land.alloc.data.table_ssp_default_updateall_orifor <- read.csv(paste0(input_dir,"aggregated.land.allocation.",scenario,"_v3_classic_gfdl_default_updateall_orifor_fixed.csv"))%>%
  mutate(scenario="default_updateall_orifor")

land.alloc.data.table_ssp_2015co2_updateall <- read.csv(paste0(input_dir,"aggregated.land.allocation.",scenario,"_v3_classic_gfdl_2015co2_updateall_fixed.csv"))%>%
  mutate(scenario="co22015_updateall")

land.alloc.data.table_ssp_2015co2_updateall_orifor <- read.csv(paste0(input_dir,"aggregated.land.allocation.",scenario,"_v3_classic_gfdl_2015co2_updateall_orifor_fixed.csv"))%>%
  mutate(scenario="co22015_updateall_orifor")

land.alloc.data.table_ssp_ssp585ndep_updateall <- read.csv(paste0(input_dir,"aggregated.land.allocation.",scenario,"_v3_classic_gfdl_ssp585ndep_updateall_fixed.csv"))%>%
  mutate(scenario="ssp585ndep_updateall")

land.alloc.data.table_ssp_ssp585ndep_updateall_orifor <- read.csv(paste0(input_dir,"aggregated.land.allocation.",scenario,"_v3_classic_gfdl_ssp585ndep_updateall_orifor_fixed.csv"))%>%
...
# Sum Tundra, Urban, Rock and desert
  others_sum <- land.alloc.data.table_combine_s7 %>%
    ...
    select(region, year, landleaf, everything())
...
write.csv(merge_alltype%>% select(-c("key","reg_id","geometry")),paste0(
    figure_save_path,
...
check1<-merge_alltype%>%
  dplyr::select(region, landtype, year,CO2_diff)
