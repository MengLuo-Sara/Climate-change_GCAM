...
process_land_allocation <- function(data) {
  ...
  ##### aggregated.land.allocation_combines_SSP370_s6->data
  ...
  long_aggregated_land_allocation_forrole_diff <- data %>%
    ...
    mutate(classic_gfdl_default_updateall_default_updateall_orifor = classic_gfdl_default_updateall - classic_gfdl_default_updateall_orifor,
           classic_ukesm_default_updateall_default_updateall_orifor = classic_ukesm_default_updateall - classic_ukesm_default_updateall_orifor,
           classic_ukesm_gfdl_diff_default_updateall_orifor = classic_ukesm_default_updateall_default_updateall_orifor-classic_gfdl_default_updateall_default_updateall_orifor) %>%
    dplyr::select(-c(5:11)) %>%
    pivot_longer(cols = classic_ukesm_gfdl_diff_default_updateall_orifor:classic_ukesm_gfdl_diff_default_updateall_orifor, 
                 names_to = "case", 
                 values_to = "value") %>%
    distinct()
...
plot_data_for_rol_diff_combine <-rbind(plot_data_SSP126$long_aggregated_land_allocation_forrole_diff,
                                          plot_data_SSP370$long_aggregated_land_allocation_forrole_diff,
                                          plot_data_SSP585$long_aggregated_land_allocation_forrole_diff)%>%
  mutate(scenario = recode(scenario, "2p6" = "SSP126", "7p0" = "SSP370", "8p5" = "SSP585"))%>%
  pivot_wider(names_from = scenario,values_from = value)

write.csv(plot_data_for_rol_diff_combine,paste0(figure_save_path,"plot_data_for_SSPs_forcings_diff_combine_",version_name,".csv"))


plot_data_for_rol_diff_combine_2100<-plot_data_for_rol_diff_combine%>%
  filter(year==2100)
write.csv(plot_data_for_rol_diff_combine_2100,paste0(figure_save_path,"Fig12_plot_data_for_rol_diff_combine_2100.csv"))
```

```{r color}

u_type<-unique(plot_data_for_rol_diff_combine$landleaf)

# Convert to factor and set levels in the desired order
plot_data_for_rol_diff_combine$landtype <- factor(plot_data_for_rol_diff_combine$landleaf, levels = c(u_type[5], u_type[9],u_type[4],u_type[8],u_type[2],u_type[1],u_type[3],u_type[7],u_type[6]))
...
create_ssp_plot <- function(data, y_column, y_limits, title, x_label , y_label = "",a_label,h_just=11.5, show_legend = TRUE, plot_margins = margin(1, 0.4, 0, 0, "cm"),if_ylab=FALSE,line_size = 1.5,ysize=0) {
...
write.csv(plot_data_for_rol_diff_combine,paste0(figure_save_path,"F12_plot_data_for_rol_diff_combine.csv"))
...
#################################
## plot
p1_SSP126_for <- create_ssp_plot(
  data = plot_data_for_rol_diff_combine,
  y_column = "SSP126",
  y_limits = c(-1000,500),
  title = "SSP126",
  x_label = "",
...
plot_combined_SSPS_lu_for <-

     plot_grid(
      plotlist = plot_list_SSPS_lu_for,
      ncol = 4,
      align = "v",
...
base_filename5 <- paste0("Fig12_",plot_var_name,"_3SSPs_forcings_diff_group_forrole_gfdl")
plot_width <- 25 * 0.5
plot_height<- 8 * 0.5
...
save_plot_in_formats(
  plot = plot_combined_SSPS_lu_for,
