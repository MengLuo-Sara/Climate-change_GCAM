...
# Read data
total_climate_forcing_SSP_combines_ukesm <- read_and_preprocess("total.climate.forcing_", csv_save_path)
aggregated_land_allocation_SSP_combines_ukesm <- read_and_preprocess("aggregated.land.allocation_", csv_save_path)

# Aggregate data by different land types
aggregated_land_allocation_combines_ukesm_s2 <- aggregate_land_type(aggregated_land_allocation_SSP_combines_ukesm, c("crops", "otherarable"), "Cropland")
aggregated_land_allocation_combines_ukesm_s3 <- aggregate_land_type(aggregated_land_allocation_combines_ukesm_s2, c("Softwood_Forest", "Hardwood_Forest"), "Managed forest")
aggregated_land_allocation_combines_ukesm_s4 <- aggregate_land_type(aggregated_land_allocation_combines_ukesm_s3, c("UnmanagedSoftwood_Forest", "UnmanagedHardwood_Forest"), "Unmanaged forest")
aggregated_land_allocation_combines_ukesm_s5 <- aggregate_land_type(aggregated_land_allocation_combines_ukesm_s4, c("rock and desert", "urban", "tundra"), "Others")

# Rename landleaf types
aggregated.land.allocation_combines_ukesm_s6 <- rename_landleaf(aggregated_land_allocation_combines_ukesm_s5)%>%
  rename(ukesm_default_updateall = ukesmdefault_updateall)
...
process_land_allocation <- function(data) {
  ...
  # data<-aggregated.land.allocation_combines_ukesm_s6
  ...
  long_aggregated_land_allocation_climate_var_diff <- data %>%
    group_by(Units, scenario, landleaf, year) %>%
    # rename(ukesm_ssp585ndep_updateall)%>%
    summarise(across(ori:ukesm_ssp585ndep_updateall, sum, na.rm = TRUE)) %>%
    ungroup() %>%
    mutate(ukesm_updateall_co2_for = (ukesm_default_updateall-ukesm_2015co2_updateall) - (ukesm_default_updateall_orifor-ukesm_2015co2_updateall_orifor),
    ukesm_updateall_ndep_for = (ukesm_ssp585ndep_updateall-ukesm_default_updateall) - (ukesm_ssp585ndep_updateall_orifor-ukesm_default_updateall_orifor),
    ukesm_updateall_precip_temp_for = (ukesm_2015co2_updateall - ori)-(ukesm_2015co2_updateall_orifor - ori)) %>%
    dplyr::select(-c(5:11)) %>%
    # pivot_longer(cols = classic_ukesm_default_updateall_ori:classic_ukesm_default_updateall_orifor_ori, 
    #              names_to = "case", 
    #              values_to = "value") %>%
    distinct()
  ...
  long_aggregated_land_allocation_climate_var_diff_re <- data %>%
    ...
    ungroup() %>%
    mutate(
      ukesm_updateall_co2_for = if_else((ukesm_default_updateall_orifor-ukesm_2015co2_updateall_orifor) == 0, 0, (((ukesm_default_updateall-ukesm_2015co2_updateall) / (ukesm_default_updateall_orifor-ukesm_2015co2_updateall_orifor)) - 1)*100),
    ukesm_updateall_ndep_for = if_else((ukesm_ssp585ndep_updateall_orifor-ukesm_default_updateall_orifor) == 0, 0, (((ukesm_ssp585ndep_updateall-ukesm_default_updateall) / (ukesm_ssp585ndep_updateall_orifor-ukesm_default_updateall_orifor)) - 1)*100),
    ukesm_updateall_precip_temp_for = if_else((ukesm_2015co2_updateall_orifor - ori) == 0, 0, (((ukesm_2015co2_updateall - ori) / (ukesm_2015co2_updateall_orifor - ori)) - 1) *100)
    ) %>%
    dplyr::select(-c(5:11)) %>%
    # pivot_longer(cols = classic_ukesm_default_updateall_ori:classic_ukesm_default_updateall_orifor_ori, 
    #              names_to = "case", 
...
plot_data_ukesm <- process_land_allocation(aggregated.land.allocation_combines_ukesm_s6)

plot_data_land_allocation <-plot_data_ukesm$long_aggregated_land_allocation
plot_data_climate_var_diff_combine <-plot_data_ukesm$long_aggregated_land_allocation_climate_var_diff
plot_data_climate_var_diff_re_combine <-plot_data_ukesm$long_aggregated_land_allocation_climate_var_diff_re

# plot_data_for_rol_re_diff_combine <-rbind(plot_data_ukesm$long_aggregated_land_allocation_forrole_diff_re)%>%
...
GC_conponent_LUC_contribution<-plot_data_climate_var_diff_combine%>%
  ...
  filter(year==2100)%>%
  mutate(total_change = ukesm_updateall_co2_for+ukesm_updateall_ndep_for+ukesm_updateall_precip_temp_for)%>%
  group_by(year)%>%
  summarise(
    abschange_CO2 = sum(abs(ukesm_updateall_co2_for), na.rm = TRUE),
    abschange_ndep = sum(abs(ukesm_updateall_ndep_for), na.rm = TRUE),
    abschange_pt = sum(abs(ukesm_updateall_precip_temp_for), na.rm = TRUE),
...
write.csv(GC_conponent_LUC_contribution,paste0(figure_save_path,"F9_GC_conponent_LUC_contribution_ukesm_2100.csv"))


write.csv(plot_data_climate_var_diff_combine,paste0(figure_save_path,"F9_plot_data_climate_var_diff_ukesm_combine.csv"))

check_CC_diff_2100<-plot_data_climate_var_diff_combine%>%
  filter(year==2100)
write.csv(check_CC_diff_2100,paste0(figure_save_path,"F9_check_CC_diff_ukesm_2100.csv"))
#################################
## plot
p1_co2_diff <- create_ssp_plot_CO2(
  data = plot_data_climate_var_diff_combine,
  y_column = "ukesm_updateall_co2_for",
...
p1_precip_temp_diff <- create_ssp_plot(
  data = plot_data_climate_var_diff_combine,
  y_column = "ukesm_updateall_precip_temp_for",
  y_limits = c(-2250, 1000),
...
p1_ndep_diff <- create_ssp_plot(
  data = plot_data_climate_var_diff_combine,
  y_column = "ukesm_updateall_ndep_for",
  y_limits = c(-2250, 1000),
  title = "N deposition",
  x_label = "",
  y_label = "", # No y-axis label needed for difference plots,
...
base_filename5 <- paste0("Fig9_",plot_var_name,"_climate_vars_impact_for_group_ukesm")
plot_width <- 25 * 0.525
plot_height<- 8 * 0.5
...
base_filename_re5 <- paste0("Fig9_",plot_var_name,"_climate_vars_re_impact_for_group_ukesm")
plot_width <- 25 * 0.525
plot_height<- 8 * 0.5
