...
result_files <- get_file_paths(havenondep, type_id, npp_dir_path)
...
for (type_id in 1:4) {
  ...
  for (data_idx in 2:2) {
    ...
    #############################################################################
    ## save PFT data ############################################################
    # since the 2015soc-from-histsoc use fixed land use 
    # then we only need to save one year's data
    # I choose to select year 2015 here
    
    ISIMIP_gcam_pft_2015 <- ISIMIP_gcam_pft %>%
      filter(year_label=="2015")%>%
      group_by()
    
    # ori_mapping<-read.csv("D:/E/new laptop2/PHD/phd_dissertation/climate_FPC_GCAM/Code/R/GCAM_mapping/gcam2glm_mapping.csv")%>%
    #   filter(center_lon==36.75)%>%
    #   filter(center_lat==-2.75)
    # subregion_list<-read.csv(paste0(root, "E/new laptop2/PHD/phd_dissertation/climate_FPC_GCAM/Data/ISIMIP3b_processed/GCAM_subregion_mean/classic_gfdl-esm4_w5e5_ssp126_2015soc-from-histsoc_default_fullscalerc3c4_agb_v3.csv"))%>%
    #   dplyr::select(region,subregion,out_reg_code,out_lu_code)
    # 
    # 
    # subregion_list2 <-
    #   read.csv(
    #     paste0(
    #       root,
    #       "E/new laptop2/PHD/phd_dissertation/climate_FPC_GCAM/Data/GCAM_carbondensity/agb_period_2020 - Copy.csv"
    #     )
    #   ) %>%
    #   mutate(
    #     word_between = str_extract(child.nodes, "(?<=_)[^_]+(?=_)"),
    #     word_after = str_extract(child.nodes, "(?<=_).*")
    #   ) %>%
    #   mutate(basin = ifelse(is.na(word_between), word_after, word_between)) %>%
    #   dplyr::select(region, basin) %>%
    #   unique()
    # 
    # # Get the total number of rows in the dataframe
    # total_rows <- nrow(subregion_list2)
    # 
    ...
    ISIMIP_gcam_pft_2015_combined <- ISIMIP_gcam_pft_2015 %>%
  mutate(
    pft_area = ifelse(
      is.na(pft) | is.na(weight) | is.na(center_lat),
      NA,
      pft * weight * cos(center_lat * pi / 180)
    )
  ) %>%
  filter(!is.na(pft_area)) %>%  # Changed to filter based on pft_area
  group_by(out_reg_code, out_lu_code) %>%
  summarize(pft_area_total = sum(pft_area, na.rm = TRUE)) %>%
  ungroup()  # Ungroup after summarizing

    
     write.csv(
      ISIMIP_gcam_pft_2015_combined,
      paste0(
        out_main_path,
        "/Area_weight_",
        broad_type,
