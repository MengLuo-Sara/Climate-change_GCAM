...
process_land_allocation <- function(data) {
  ...
  long_aggregated_land_allocation_forrole_diff_gfdl <- data %>%
    ...
    summarise(across(ori:classic_gfdl_default_updateall, sum, na.rm = TRUE)) %>%
    ungroup() %>%
    mutate(classic_gfdl_default_updateall_default_updateall_ori = classic_gfdl_default_updateall - ori) %>%
    dplyr::select(-c(5:7)) %>%
    pivot_longer(cols = classic_gfdl_default_updateall_default_updateall_ori:classic_gfdl_default_updateall_default_updateall_ori, 
                 names_to = "case", 
                 values_to = "value") %>%
    distinct()
  ...
  long_aggregated_land_allocation_forrole_diff_ukesm <- data %>%
    ...
    ungroup() %>%
    mutate(classic_ukesm_default_updateall_default_updateall_ori = classic_ukesm_default_updateall - ori) %>%
    dplyr::select(-c(5:9)) %>%
    pivot_longer(cols = classic_ukesm_default_updateall_default_updateall_ori:classic_ukesm_default_updateall_default_updateall_ori, 
                 names_to = "case", 
                 values_to = "value") %>%
    distinct()
...
plot_data_CC_all_veg_diff_combine_gfdl <-rbind(plot_data_SSP126$long_aggregated_land_allocation_forrole_diff_gfdl,                                          plot_data_SSP370$long_aggregated_land_allocation_forrole_diff_gfdl,                                          plot_data_SSP585$long_aggregated_land_allocation_forrole_diff_gfdl)%>%
  mutate(scenario = recode(scenario, "2p6" = "SSP126", "7p0" = "SSP370", "8p5" = "SSP585"))%>%
  pivot_wider(names_from = scenario,values_from = value)

plot_data_CC_all_veg_diff_combine_ukesm <-rbind(plot_data_SSP126$long_aggregated_land_allocation_forrole_diff_ukesm,                                          plot_data_SSP370$long_aggregated_land_allocation_forrole_diff_ukesm,                                          plot_data_SSP585$long_aggregated_land_allocation_forrole_diff_ukesm)%>%
  mutate(scenario = recode(scenario, "2p6" = "SSP126", "7p0" = "SSP370", "8p5" = "SSP585"))%>%
  pivot_wider(names_from = scenario,values_from = value)
...
u_type<-unique(plot_data_CC_all_veg_diff_combine_gfdl$landleaf)
...
## abs diff #############################
plot_data_CC_all_veg_diff_combine_gfdl$landtype <- factor(plot_data_CC_all_veg_diff_combine_gfdl$landleaf, levels = c(u_type[5], u_type[9],u_type[4],u_type[8],u_type[2],u_type[1],u_type[3],u_type[7],u_type[6]))

plot_data_CC_all_veg_diff_combine_ukesm$landtype <- factor(plot_data_CC_all_veg_diff_combine_ukesm$landleaf, levels = c(u_type[5], u_type[9],u_type[4],u_type[8],u_type[2],u_type[1],u_type[3],u_type[7],u_type[6]))
...
p1_SSP126_for_gfdl <- create_ssp_plot_abs(
  data = plot_data_CC_all_veg_diff_combine_gfdl,
  y_column = "SSP126",
  y_limits = c(-4000,4000),
  title = "SSP126",
  x_label = "",
...
base_filename4_gfdl <- paste0("Fig18_",plot_var_name,"_3SSPs_abs_CC_allveg_impact_group_gfdl")
plot_width <- 25 * 0.5
plot_height<- 8 * 0.5
...
p1_SSP126_for_ukesm <- create_ssp_plot_abs(
  data = plot_data_CC_all_veg_diff_combine_ukesm,
  y_column = "SSP126",
  y_limits = c(-4000,4000),
  title = "SSP126",
  x_label = "",
...
base_filename4_ukesm <- paste0("Fig18_",plot_var_name,"_3SSPs_abs_CC_allveg_impact_group_ukesm")
plot_width <- 25 * 0.5
plot_height<- 8 * 0.5
