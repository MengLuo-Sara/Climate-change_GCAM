...
process_land_allocation <- function(data) {
  ...
  # aggregated.land.allocation_combines_SSP126_s6->data
  
  
  long_aggregated_land_allocation_update_all_gfdl <- data %>%
    group_by(Units, scenario, landleaf, year) %>%
    summarise(across(ori:classic_gfdl_default_updateall, sum, na.rm = TRUE)) %>%
    ungroup() %>%
    dplyr::select(-c(5:6)) %>%
    pivot_longer(cols = classic_gfdl_default_updateall, 
                 names_to = "case", 
                 values_to = "value") %>%
    distinct() 
  
  long_aggregated_land_allocation_update_all_ukesm <- data %>%
    group_by(Units, scenario, landleaf, year) %>%
    summarise(across(classic_ukesm_default_updateall, sum, na.rm = TRUE)) %>%
    ungroup() %>%
    pivot_longer(cols = classic_ukesm_default_updateall, 
                 names_to = "case", 
                 values_to = "value") %>%
    distinct() 
  ...
  return(list(
    long_aggregated_land_allocation_update_all_gfdl = long_aggregated_land_allocation_update_all_gfdl,
    long_aggregated_land_allocation_update_all_ukesm = long_aggregated_land_allocation_update_all_ukesm,
...
plot_data_update_all_combine_gfdl <-rbind(plot_data_SSP126$long_aggregated_land_allocation_update_all_gfdl,                                          plot_data_SSP370$long_aggregated_land_allocation_update_all_gfdl,                             plot_data_SSP585$long_aggregated_land_allocation_update_all_gfdl)%>%
  mutate(scenario = recode(scenario, "2p6" = "SSP126", "7p0" = "SSP370", "8p5" = "SSP585"))%>%
  pivot_wider(names_from = scenario,values_from = value)



plot_data_update_all_combine_ukesm <-rbind(plot_data_SSP126$long_aggregated_land_allocation_update_all_ukesm,                                          plot_data_SSP370$long_aggregated_land_allocation_update_all_ukesm,                             plot_data_SSP585$long_aggregated_land_allocation_update_all_ukesm)%>%
  mutate(scenario = recode(scenario, "2p6" = "SSP126", "7p0" = "SSP370", "8p5" = "SSP585"))%>%
  pivot_wider(names_from = scenario,values_from = value)
...
write.csv(plot_data_update_all_combine_gfdl,paste0(figure_save_path,"Fig4_plot_data_update_all_combine_gfdl.csv"))
write.csv(plot_data_update_all_combine_ukesm,paste0(figure_save_path,"Fig4_plot_data_update_all_combine_ukesm.csv"))
