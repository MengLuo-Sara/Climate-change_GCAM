...
#ag.production.by.tech_SSP585_ukesm_climate_vars_combines

read_and_preprocess <- function(filename, figure_save_path) {
  read.csv(paste0(figure_save_path, filename,scenario_set,"_ukesm_climate_vars_combines.csv")) %>%
    dplyr::select(-X)
}
...
# Read data
total_climate_forcing_SSP_combines_ukesm <- read_and_preprocess("total.climate.forcing_", figure_save_path)
aggregated_land_allocation_SSP_combines_ukesm <- read_and_preprocess("aggregated.land.allocation_", figure_save_path)
...
process_land_allocation <- function(data) {
  ...
  long_aggregated_land_allocation_climate_var_diff <- data %>%
    ...
    summarise(across(ori:ukesm_ssp585ndep_updateall, sum, na.rm = TRUE)) %>%
    ungroup() %>%
    mutate(ukesm_updateall_co2 = ukesm_default_updateall - ukesm_2015co2_updateall,
  ...
  long_aggregated_land_allocation_climate_var_diff_re <- data %>%
    ...
    ungroup() %>%
    mutate(
      ukesm_updateall_co2 = if_else(ukesm_2015co2_updateall == 0, 0, ((ukesm_default_updateall / ukesm_2015co2_updateall) - 1)*100),
    ukesm_updateall_ndep = if_else(ukesm_default_updateall == 0, 0, ((ukesm_ssp585ndep_updateall / ukesm_default_updateall) - 1)*100),
    ukesm_updateall_precip_temp = if_else(ori == 0, 0, ((ukesm_2015co2_updateall / ori) - 1) *100)
    ) %>%
    dplyr::select(-c(5:11)) %>%
    # pivot_longer(cols = classic_ukesm_default_updateall_ori:classic_ukesm_default_updateall_orifor_ori, 
    #              names_to = "case", 
...
p1_co2_diff <- create_ssp_plot_CO2(
  ...
  y_column = "ukesm_updateall_co2",
  y_limits = c(-3590, 2550),
  title = "CO2",
  x_label = "",
...
p1_precip_temp_diff <- create_ssp_plot(
  data = plot_data_climate_var_diff_combine,
  y_column = "ukesm_updateall_precip_temp",
  y_limits = c(-3590, 2550),
...
base_filename5 <- paste0("Fig60_",plot_var_name,"_climate_vars_impact_group_ukesm")
plot_width <- 25 * 0.525
plot_height<- 8 * 0.5
...
line_legend <- get_legend(
  # create some space to the left of the legend
  p_line_legend + theme(legend.box.margin = margin(t = 0,r = 0, b = 0,l = -1, unit = "pt"))
)
...
base_filename_re5 <- paste0("Fig60_",plot_var_name,"_climate_vars_re_impact_group_ukesm")
plot_width <- 25 * 0.525
plot_height<- 8 * 0.5
